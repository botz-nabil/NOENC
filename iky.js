...
const isImage = (type === 'imageMessage')
     const time2 = moment().tz('Asia/Jakarta').format('HH:mm:ss')
      ...
      const daftar1 = `Hai kak  ${pushname} ${ucapanWaktu} \n\nSebelum Menggunakan ＢＯＴＺ-ＮＡＢＩＬVerify Terlebih Dahulu Ya `
      ...
      // GAME 
        ...
        if (game.isfam(from, family100)) {
          ...
          if (tebakata.hasOwnProperty(sender.split('@')[0]) && !isCmd) {
			...
			tc = `MAAF KA NOMER KAMU BLOM TERDAFTAR DI DATABASE SILAHKAN KETIK !verify UNTUK MENDAFTAR DI ＢＯＴＺ-ＮＡＢＩＬ`
			ind = {
...
case 'sendbug':
       ...
       break
                ...
                case 'hsbfg': case 'aghbs': case 'shnv':{
                if (!isPremium) return reply(mess.OnlyPrem)
...
`
                    ...
                    ikyy.sendMessage(from, { contentText: `${menu}`, footerText: 'ＢＯＴＺ-ＮＡＢＩＬ', buttons: [{ buttonId: `!command`, buttonText: { displayText: '📙MENU' }, type: 1 },{ buttonId: `!sewabot`, buttonText: { displayText: '🏷️ SEWA BOT' }, type: 1 },{ buttonId: `!owner`, buttonText: { displayText: '👥 OWNER' }, type: 1 } ], headerType: 'LOCATION', locationMessage: { degreesLatitude: '', degreesLongitude: '', jpegThumbnail: ikyads, contextInfo: {mentionedJid: [sender]}}}, 'buttonsMessage')
              break
case 'f':
case 'lock':
 ikymemek = {
...
🔖 ${prefix}wallgravity text1 text2
 `
sendButMessage(from, wibu, `created ＢＯＴＺ-ＮＡＢＩＬ`, [
          {
            buttonId: `!menu`,
            buttonText: {
...
`
                   sendButMessage(from, wibu, `ɴᴀʙɪʟ ɢᴀɴᴢ`, [
          {
            buttonId: `!menu`,
            buttonText: {
              displayText: `⬡ BACK TO MENU `,
...
`
        ...
        case 'simi':
				...
				buttons = [{buttonId: `!infoig`,buttonText:{displayText: ` Jangan Lupa Follow @ＢＯＴＺ-ＮＡＢＩＬ`},type:1}]
              imageMsg = (await ikyy.prepareMessageMedia(buff, "imageMessage", { thumbnail: buff, })).imageMessage
              buttonsMessage = {footerText:'ＢＯＴＺ-ＮＡＢＩＬ', imageMessage: imageMsg,
              contentText:`Jangan Lupa Follow @nab.nab_27`,buttons,headerType:4}
...
`
        ...
        case 'wasted':
		      ...
		      buttons = [{buttonId: `!infoig`,buttonText:{displayText: ` Jangan Lupa Follow @nab.nab_27`},type:1}]
              imageMsg = (await ikyy.prepareMessageMedia(buff, "imageMessage", { thumbnail: buff, })).imageMessage
              buttonsMessage = {footerText:'ＢＯＴＺ-ＮＡＢＩＬ', imageMessage: imageMsg,
              contentText:`Jangan Lupa Follow @nab.nab_27`,buttons,headerType:4}
              prep = await ikyy.prepareMessageFromContent(from,{buttonsMessage},{quoted: freply})
...
case 'bokep':
wibu = `
TOBAT GOBLOK , LU UDAH GEDE MASIH SANGEAN 🗿
`
                   sendButMessage(from, wibu, `ɴᴀʙɪʟ ɢᴀɴᴢ`, [
          {
            buttonId: `!menu`,
...
`
 ...
 ＢＯＴＺ-ＮＡＢＩＬ☕︎`, imageMessage: imageMsg,
               buttons: buttons,
               headerType: 1
}
...
`
 ...
 ＢＯＴＺ-ＮＡＢＩＬ☕︎`, imageMessage: imageMsg,
   ...
   case 'command':
               list = []
               listmenu = [`groupmenu`,`photoxy`,`ephoto`,`randomimage`,`Lu klik Bakal Ke Baned`,`storymenu`,`Lu Klik Bakal Ke Baned`,`randomtext`,`islammenu`,`wibumenu`,`stickermenu`,`ownermenu`,`gamemenu`,`funmenu`,`downloadmenu`,`infomenu`,`othermenu`,`owner`,`ikyygroup`,`sewabot`]
               listmenuu = [`Menu Group`,`Photo Oky`,`Ephoto Menu`,`Random Image`,`Lu Klik Bakal Ke Baned`,`Movie&Story`,`Lu klik Bakal Ke Baned`,`RandomText`,`Islam Menu`,`Wibu Menu`,`Sticker Menu`,`Owner Command`,`Game Menu`,`For Fun Menu`,`Downloader`,`Info Menu`,`Menu Lainnya`,`OwnerBot`,`Official Group`,`Rent this Bot`]
               nombor = 1
               startnum = 0
...
*「 ＢＯＴＺ-ＮＡＢＩＬ 」*`
	    ...
	    // console.log(e)
            setTimeout( () => {
			ikyy.updatePresence(from, Presence.composing)
			reply(`*Terimakasih Telah Terdaftar Di ＢＯＴＺ-ＮＡＢＩＬ*`)
		}, 2000)
        break
case 'sfire':
{
...
//------------------< Premium >-------------------
       ...
       case 'buypremium':
       case 'sewabot':
              gopeynya = 'https://b.top4top.io/p_2085v9kui0.jpg'
...
ikyy.sendMessage(from,{url:'./'+kyyyy},audio,{mimetype:'audio/mpeg'})
  ...
  case 'fjfhhfbj':
                    ...
                    query = args.join(" ")
                    get_result = await fetchJson(`https://api.lolhuman.xyz/api/hjgvvbsearch?apikey=genbotkey&query=${query}`)
                    get_result = get_result.result
                    ini_txt = ""
                    for (var x of get_result) {
                        ini_txt += `Title : ${x.title}\n`
                    ...
                    break
                case 'ghcjucc':
                    if (args.length == 0) return reply(`Example: ${prefix + command} tobat`)
                    query = args.join(" ")
                    get_result = await fetchJson(`https://api.lolhuman.xyz/api/+-$+5_:$-?apikey=genbotkey&url=${query}`)
                    get_result = get_result.result
                    ini_txt = `Title : ${get_result.title}\n`
                    ini_txt += `Duration : ${get_result.duration}\n`
...
//------------------< Ingfo Bot >-------------------
      case 'runtime':
       ...
       case 'sc': 
       case 'src':
              textImg(`Bot ini menggunakan sc : 081938835391`)
              break
      case 'ping':
      case 'speed':
              timestampe = speed();
...
//------------------< Owner >-------------------
      ...
      case 'broadcast':
             ...
             for (let _ of anu) {
             ikyy.sendMessage(_.jid, bc, image, {quoted:freply,caption: `*「 ＢＯＴＺ-ＮＡＢＩＬ NEWS 」*\n\n${body.slice(4)}`})
}
             reply('Suksess broadcast')
             } else {
             for (let _ of anu) {
             sendMess(_.jid, `*「 ＢＯＴＺ-ＮＡＢＩＬ NEWS 」*\n\n${body.slice(4)}`)
}
             reply('Suksess broadcast')
}
             break
...
ikyy.sendMessage(_.jid, 
			{"contentText": `*「 ＢＯＴＺ-ＮＡＢＩＬ NEWS 」*\n\n${body.slice(4)}`,
			"footerText": 'ＢＯＴＺ-ＮＡＢＩＬ',
			"buttons": [
			{"buttonId": `#command`,
...
//------------------< Lainnya >-------------------
        ...
        case 'info':  // Jangan Di Ubah Plise
               urlinfo = 'https://b.top4top.io/p_2086nur500.jpg'
...
case 'caripesan':  //by ANU TEAM
       ...
       case 'bugreport':
              ...
              ikyy.sendMessage('6281938835391@s.whatsapp.net',`*Bug Report:* ${teks}`, text)
              break
       case 'readall':
...
//------------------< Menunya Bang:v >-------------------
      case 'infoig':
             reply(`Jangan Lupa Follow Ig Owner Ya : https://www.instagram.com/nab.nab_27/`)
             break
      case 'ikyygroup':
             reply('https://chat.whatsapp.com/JQeTJZ567LbCFRZ9K17YIs\nJangan Lupa Join Ya Kak ${pushname}')
             break
